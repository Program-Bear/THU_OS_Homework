...
class scheduler:
    ...
    def move_to_ready(self, expected, pid=-1):
        target = self.curr_proc
        if pid != -1:
            target = pid
        assert(self.proc_info[target][PROC_STATE] == expected)
        self.proc_info[target][PROC_STATE] = STATE_READY
        return
    ...
    def move_to_wait(self, expected):
        ...
        assert(self.proc_info[target][PROC_STATE] == expected)
        #print(
        self.proc_info[target][PROC_STATE] = STATE_WAIT
        return
    ...
    def next_proc(self, pid=-1):
        if (pid == -1):
            pid = self.curr_proc
        total_num = self.get_num_processes()
        p = pid
        
        while(((p + 1) % total_num) != pid):
            p = (p + 1) % total_num
    ...
    def run(self):
        ...
        while self.get_num_active() > 0:
            ...
            if instruction_to_execute == DO_IO:
                ft = clock_tick + 1 + self.io_length
                self.io_finish_times[self.curr_proc].append(ft)
